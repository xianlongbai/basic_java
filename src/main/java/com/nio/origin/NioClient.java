        ...
        while(true){
            ...
            while(ite.hasNext()){
                ...
                if(key.isConnectable()){
                    ...
                    channel.configureBlocking(false);
                    //向服务器发送消息
                    //ByteBuffer buf = ByteBuffer.allocate(1024);//jvm分配内存
                    ByteBuffer buf = ByteBuffer.allocateDirect(1024);//堆外分配内存
                    buf.put("send message to server.".getBytes());
                    buf.flip();
                    channel.write(buf);
